...
const TasksWithDragAndDrp = () => {
    ...
    const handleDrop = (e) => {
        ...
        const dropTargetIndex = tasks.length; // Drop at the end of the list

        // Reorder tasks
        updatedTasks.splice(draggedTaskIndex, 1);
        updatedTasks.splice(dropTargetIndex, 0, tasks[draggedTaskIndex]);
    ...
    const tasksList = tasksToRender.map((task, index) => (
        <li
            className='task'
            key={task.id}
            draggable
...
export default TasksWithDragAndDrp;
